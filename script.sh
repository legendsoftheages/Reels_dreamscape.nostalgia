...
# Directories (Using your clean relative paths)
TMP=$(mktemp -d)
INPUT_DIR="./reels"
AUDIO_DIR="./audio"
...
# Fallback if your Inter-Black.ttf isn't in the folder
...
echo "🎨 Applying Visuals..."
TOTAL=$(wc -l < "$QUOTES_FILE" | xargs)
line=$((RANDOM % TOTAL + 1))

# CLEANING: Perl nukes the "ghost boxes"
...
[v_l]drawtext=fontfile='${FONT}':textfile='$TMP/quote.txt':fontcolor=white:fontsize=38: \
...
out_file="$OUTPUT_DIR/${safe}.mp4"

ffmpeg -i "$VISUAL_MASTER" -i "$AUDIO_FILE" \
  -filter_complex "[1:a]afade=t=out:st=${FADE_VAL}:d=2[aud]" \
  -map 0:v -map "[aud]" -c:v copy -c:a aac -shortest "$out_file" -y -loglevel warning
